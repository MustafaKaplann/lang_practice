...
export default function QuizPage() {
  ...
  const pool = useMemo(() => {
    ...
    return allWords.filter((w) => {
      if (w.sublist === undefined) return selectedSublists.size === 10;
  ...
  const startQuiz = useCallback(() => {
    ...
    const qs = buildQuiz(pool, total, direction);
    setQuestions(qs);
    setIndex(0);
    setPickedIndex(null);
    setAnswered([]);
...
function Setup(props: {
  ...
  setDirection: (d: DirectionSetting) => void;
  countChoice: CountChoice;
  ...
  const toggle = (n: number) => {
    ...
    if (next.size === 0) next.add(n); // at least one
    ...
    <div className="space-y-6">
      ...
      <Field label="Sublist (çoklu seçim)">
        <div className="flex flex-wrap gap-2">
          ...
          {SUBLISTS.map((n) => {
            const active = selectedSublists.has(n);
            return (
              <button
                key={n}
                type="button"
      ...
      {!canStart && (
        <p className="text-red-400 text-sm">
          Seçili filtrelerle yeterli kelime yok, en az 4 gerekli.
        </p>
      )}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-8 text-center">
            ...
            <div
              className={
                current.direction === "en-tr"
                  ? "font-heading text-4xl sm:text-5xl font-bold text-slate-100"
                  : "text-xl sm:text-2xl text-slate-100 leading-snug"
              }
            >
              {current.prompt}
            </div>
      ...
      {wrongs.length > 0 && (
        <div className="space-y-3">
          ...
          <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
            {wrongs.map((a, i) => {
              const w = wordById.get(a.question.wordId);
              if (!w) return null;
              return (
                <div
                  key={i}
