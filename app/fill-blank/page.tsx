...
export default function FillBlankPage() {
  ...
  }, [eligible, selectedSublists, onlyUnknown]);

  const startQuiz = useCallback(() => {
    const total =
      countChoice === "all" ? pool.length : Math.min(countChoice, pool.length);
  ...
  const revealed =
    difficulty === "easy" ? pickedIndex !== null : submittedHard;
  ...
  const submitHard = useCallback(() => {
    ...
    const correct =
      typed.trim().toLowerCase() === current.word.toLowerCase();
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-slate-400">
        ...
        <span className="text-slate-200 font-medium">{eligibleCount}</span> kelime
        mevcut.
      </p>
      ...
      <Field label="Zorluk">
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`${pillCls(difficulty === "easy")} ${
              easyDisabled ? "opacity-50 cursor-not-allowed" : ""
            }`}
          >
            Kolay (4 şık)
          </button>
          <button
            ...
            className={pillCls(difficulty === "hard")}
          >
            Zor (yazma)
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {COUNTS.map((c) => (
            <button
              key={String(c)}
              type="button"
              aria-pressed={countChoice === c}
              onClick={() => setCountChoice(c)}
              className={pillCls(countChoice === c)}
            >
              {c === "all" ? "Tümü" : c}
            </button>
          ))}
      ...
      {!canStart && (
        <p className="text-red-400 text-sm">
          Bu filtrelerle yeterli kelime yok, en az 4 gerekli.
        </p>
      )}
...
function Playing(props: {
  ...
  const hardCorrect =
    submittedHard && typed.trim().toLowerCase() === current.word.toLowerCase();
  ...
  return (
    <div className="space-y-6">
          ...
          {difficulty === "easy" ? (
            <div className="space-y-2">
              {current.options.map((opt, i) => {
                ...
                if (!revealed) {
                  cls +=
                    "bg-slate-900 border-slate-800 text-slate-200 hover:border-emerald-500/50 hover:bg-slate-800";
                } else if (isCorrect) {
                  cls +=
      ...
      <p className="text-center text-xs text-slate-500">
        {difficulty === "easy"
          ? "1–4: şık seç · Enter: sonraki"
          : "Enter: cevapla / sonraki"}
      </p>
    </div>
  );
}
...
function Result({
  ...
  const xp = answered.reduce(
    (sum, a) => sum + (a.correct ? (a.hintUsed ? 5 : 10) : 0),
    0,
  );
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-8 text-center space-y-4">
        ...
        <div className="font-heading text-6xl font-bold text-emerald-400">
          {correct} / {total}
        </div>
        <p className="text-slate-400">
          ...
          {hintCount > 0 && (
            <>
              {" · "}
              <span className="text-amber-300">💡 {hintCount} ipucu</span>
            </>
          )}
        </p>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm italic text-slate-400">
                    &ldquo;{parts[0]}
                    <span className="text-emerald-300 font-bold not-italic">
                      {a.q.word}
                    </span>
                    {parts[1]}&rdquo;
                  </p>
